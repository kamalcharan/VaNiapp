...
function formatChapterName(chapterId: string): string {
  // Strip prefix like 'cuet-phy-', 'phy-', 'chem-', etc. and humanize
  const stripped = chapterId
    .replace(/^cuet-(?:phy|chem|bot|zoo)-/, '')
    .replace(/^(?:phy|chem|bot|zoo)-/, '');
  return stripped
    .split('-')
...
export default function HistoryScreen() {
  ...
  const entries = useMemo(() => {
    ...
    chapterHistory.forEach((s) => {
      if (s.completedAt) {
        items.push({ type: 'chapter', session: s, date: new Date(s.completedAt) });
      }
    });
    practiceHistory.forEach((s) => {
      if (s.completedAt) {
        items.push({ type: 'practice', session: s, date: new Date(s.completedAt) });
      }
    });
  ...
  const handleChapterTap = (s: ChapterExamSession) => {
    router.push({
      pathname: '/chapter-results',
      params: {
        chapterId: s.chapterId,
        correct: String(s.correctCount ?? 0),
        total: String(s.totalQuestions),
        timeUsedMs: String(s.timeUsedMs ?? 0),
      },
    });
  };

  const handlePracticeTap = (s: PracticeExamSession) => {
    router.push({
  ...
  const FILTERS: { key: FilterMode; label: string }[] = [
    ...
    { key: 'chapter', label: `Chapter (${chapterHistory.filter((s) => s.completedAt).length})` },
    { key: 'practice', label: `Practice (${practiceHistory.filter((s) => s.completedAt).length})` },
  ];
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((entry, idx) => {
                  if (entry.type === 'chapter') {
                    ...
                    return (
                      <Pressable key={s.id} onPress={() => handleChapterTap(s)}>
                        <JournalCard delay={idx * 60} style={styles.entryCard}>
                          <View style={styles.entryTop}>
                            <View style={[styles.modeBadge, { backgroundColor: (meta?.color ?? '#6366F1') + '20' }]}>
                              <Text style={[styles.modeBadgeText, { color: meta?.color ?? '#6366F1' }]}>CHAPTER</Text>
                            </View>
                          ...
                          <Text style={[Typography.h3, { color: colors.text, marginTop: 6 }]} numberOfLines={1}>
                            {meta?.emoji ?? '\u2699\uFE0F'} {formatChapterName(s.chapterId)}
                          </Text>
                  ...
                  return (
                    <Pressable key={s.id} onPress={() => handlePracticeTap(s)}>
                      <JournalCard delay={idx * 60} style={styles.entryCard}>
                        <View style={styles.entryTop}>
