#!/usr/bin/env node
/**
 * Insert Ch3 Producer Behaviour & Supply questions into Supabase DB.
 *
 * Usage:
 *   node Qbank/CUET/economics/producer-supply/insert_questions.js
 *   node Qbank/CUET/economics/producer-supply/insert_questions.js --dry-run
 *
...
const CHAPTER_ID = 'cuet-eco-producer-supply';
...
async function main() {
  ...
  // Load all JSON question files from the directory
  const jsonFiles = readdirSync(BATCH_DIR)
    .filter(f => f.endsWith('.json') && f.startsWith('eco-ps-'))
    .sort();
  ...
  console.log(`\nEcon Ch3 Producer Behaviour & Supply — Insert${dryRun ? ' (DRY RUN)' : ''}`);
  ...
  console.log(`\nAll done! ${inserted} questions inserted for Ch3 Producer Behaviour & Supply.`);
}
