...
def main():
    ...
    migration_path = MIGRATION_DIR / "20260304_seed_all_cuet_questions.sql"

    lines = []
    lines.append("-- ==========================================================================")
    lines.append("-- Seed ALL CUET physics questions from JSON files into the database")
    lines.append(f"-- Total questions: {len(all_questions)}")
    lines.append("-- Generated by fix_and_seed.py")
    lines.append("-- ==========================================================================")
    lines.append("")
    lines.append("BEGIN;")
    lines.append("")
    ...
    lines.append("-- ── Part A: Create missing topics ────────────────────────────────────────")
    ...
    if topic_rows:
        ...
        # Remove the INSERT statement if no new topics
        lines.pop()  # remove VALUES
        lines.pop()  # remove INSERT
        lines.append("-- No new topics needed")
    lines.append("")

    # ── Part B: Re-create insert helper (if dropped by step4) ───────────────
    lines.append("-- ── Part B: Re-create batch insert function ──────────────────────────────")
    lines.append("""CREATE OR REPLACE FUNCTION insert_batch_questions_v2(questions jsonb, p_subject_id text, batch_source text)
RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
$$;""")
    ...
    # ── Part C: Insert questions in batches ──────────────────────────────────
    lines.append("-- ── Part C: Insert questions in batches ──────────────────────────────────")
    ...
    for i in range(0, len(all_questions), BATCH_SIZE):
        ...
        lines.append(f"SELECT insert_batch_questions_v2('{batch_json_escaped}'::jsonb, 'cuet-physics', 'json-seed-batch-{batch_num}');")
        lines.append("")

    # ── Part D: Cleanup ─────────────────────────────────────────────────────
    lines.append("-- ── Part D: Cleanup ──────────────────────────────────────────────────────")
    ...
    print(f"  Batches: {(len(all_questions) + BATCH_SIZE - 1) // BATCH_SIZE}")
